  ...
  describe("createServer", () => {
    ...
    it("should register CreateX402PaymentHeader tool", async () => {
      ...
      try {
        ...
        assert.ok(createHeaderTool, "CreateX402PaymentHeader tool should be registered");
    ...
    it("should have correct server metadata", async () => {
      ...
      try {
        ...
        assert.ok(tools.tools.length > 0, "Server should have registered tools");

        const toolNames = tools.tools.map(t => t.name);
        assert.ok(toolNames.includes("CreateX402PaymentHeader"), "Should have CreateX402PaymentHeader tool");
        assert.ok(toolNames.includes("LookupAddress"), "Should have LookupAddress tool");
      } finally {
        await close(server);
      }
    });
    ...
    it("should allow multiple server instances", async () => {
      ...
      assert.notStrictEqual(server1, server2, "Servers should be different instances");
      ...
      try {
        ...
        assert.strictEqual(tools1.tools.length, 2, "First server should have 2 tools");
        assert.strictEqual(tools2.tools.length, 2, "Second server should have 2 tools");
